Dissonance PUN2 Demo
====================

This demo demonstrates the core features of Dissonance voice comms with Photon Unity Networking 2:

* The Dissonance PUN2 "DissonanceSetup" prefab.
...
1. Add both "PUN Demo" and "PUN Game World" scenes to your project's build settings, and drag "PUN Demo" to the top of the list.
2. Click File -> Build and Run.
3. Once the client is running, run the Demo scene in the editor.
4. In editor, click "LAN Host(H)".
5. On the client, click "LAN Client(C)".
...
Each player prefab contains a "Photon Player" script. This script tracks the position of the player and enables positional audio playback for Dissonance voices as well as collider triggering for "Voice Broadcast Trigger" and "Voice Receipt Trigger".
