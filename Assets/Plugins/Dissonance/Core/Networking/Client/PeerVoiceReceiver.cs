...
namespace Dissonance.Networking.Client
{
    ...
    internal class PeerVoiceReceiver
    {
        ...
        public void ReceivePacket(ref PacketReader reader, DateTime utcNow)
        {
            ...
            if (IsPacketFromPreviousSession(_currentChannelSession, metadata.ChannelSession, metadata.ChannelSessionRange))
            {
        ...
        /// <returns></returns>
        private static bool IsPacketFromPreviousSession(int currentChannelSession, int packetChannelSession, int range)
        {
            ...
            var delta = currentChannelSession.WrappedDelta(packetChannelSession, range);
            return delta < 0;
        }
